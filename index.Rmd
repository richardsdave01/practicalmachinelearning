...
We see a lot of NAs and "" (empty strings). Also, some fields that we would expect to be numeric contain character data, so we investigate one of them further.
```{r check_col, include=FALSE}
table(training$kurtosis_roll_dumbbell)
```

We find 5 "DIV/0!" values (omitting the output due to space limitations), so let's reread the data, changing them to NA.
```{r read_train}
...
The first seven columns are metadata so let's remove them.
```{r remove_meta}
...
```{r read_test}
testing = read.csv("~/R/MachineLearning/pml_project/data/pml-testing.csv", stringsAsFactors = FALSE,
...
x <- train_good[,-53]
y <- train_good[,53]
fit <- train(x, y, method="rf", data=train_good, trControl=fitControl)
```
